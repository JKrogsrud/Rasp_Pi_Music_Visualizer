...
into the bucket. The alternate version bucket_freqs_avg places the average of the frequency array
between the two given endpoints into the bucketed array.
...
def bucket_freqs_avg(frequencies, buckets):
    bucketed_freqs = []
    for i in range(len(buckets)-1):
        avg_amp = sum(frequencies[buckets[i]:buckets[i+1]])/max(len(frequencies[buckets[i]:buckets[i+1]]), 1)
        bucketed_freqs.append(avg_amp)
    return bucketed_freqs
...
def bucket_chunk(amps, start, buckets):
    ...
    return bucket_freqs_avg(frequencies, buckets)
...
def scaled_frequencies(freq_bucket, max_amp, max_height):
    scaled_list = [x * max_height / max_amp for x in freq_bucket]
    return scaled_list
